...
// Query base com filtros - Buscando pacientes com ALTA (ativo = 0)
$sql = "
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
    <div>
        <h1 class="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">Altas Fugulin</h1>
        <p class="text-sm text-slate-500">Pacientes que já receberam alta do sistema de classificação.</p>
    </div>
    <a href="fugulin_lista.php" class="w-full md:w-auto bg-slate-100 hover:bg-slate-200 text-slate-600 px-6 py-3 rounded-2xl font-bold transition-all flex items-center justify-center gap-2">
...
<div class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-6">
    <form method="GET" class="flex flex-col md:flex-row gap-4">
        ...
        <?php if (!empty($search_patient) || $filter_sector): ?>
            <a href="fugulin_altas.php" class="bg-slate-100 hover:bg-slate-200 text-slate-600 px-6 py-3 rounded-2xl font-bold transition-all flex items-center justify-center">
                Limpar
            </a>
        <?php endif; ?>
    </form>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
                <?php if (empty($pacientes)): ?>
                    <tr>
                        <td colspan="6" class="px-8 py-12 text-center text-slate-400">
                            <i class="fas fa-sign-out-alt text-4xl mb-4 block opacity-20"></i>
                            Nenhum paciente no histórico de altas.
                        </td>
                    </tr>
                <?php endif; ?>
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-8 py-5 text-center">
                        <span class="bg-white border border-slate-200 text-slate-600 px-3 py-1 rounded-lg text-xs font-black">
                    ...
                    <td class="px-8 py-5 text-center flex items-center justify-center gap-2">
                        ...
                        <a href="fugulin_action.php?acao=readmitir&id=<?php echo $p['id']; ?>" 
                           onclick="return confirm('Deseja re-admitir o paciente <?php echo cleanInput($p['nome']); ?> na monitoria ativa?')"
                           class="w-10 h-10 inline-flex items-center justify-center bg-green-50 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all shadow-sm" title="Re-admitir">
